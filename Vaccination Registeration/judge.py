...
class Tasks:
	def __init__(self, dir_, port):
		self.read_port = port+1000
		self.read_port2 = port+1003

		self.write_port = port+1001
		self.write_port2 = port+1002
		self.dir_ = dir_
	...
	def task6(self):
		...
		try:
			telnet_client = TelnetConnection("localhost", self.write_port)
			telnet_client = telnet_client.telnet()
			telnet_client2 = TelnetConnection("localhost", self.write_port)
		...
		try:
			...
			write_proc2.wait()			
	def task7(self):
		task7_str1 = "Your preference order is AZ > BNT > Moderna.\n"
		task7_str2 = "Preference order for 902001 modified successed, new preference order is Moderna > BNT > AZ.\n"
		task7_str3 = "Locked\n"
		write_proc = self.start_server("./write_server", self.write_port)
		read_proc = self.start_server("./read_server", self.read_port)
		try:
		...
		try:
			gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server output)"+gets)
			assert (gets == self.str1), '\033[91m'+"task7-1 Failed. Should be\n"+'\033[0m'+self.str1+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			gets = telnet_client2.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server2 output)"+gets)
			assert (gets == self.str1), '\033[91m'+"task7-1 Failed. Should be\n"+'\033[0m'+self.str1+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			print("\33[3m"+"(Client1 side input) " +"\33[0m"+"902001")
			...
			gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server output)"+gets)
			assert (gets == task7_str1), '\033[91m'+"task7-1 Failed. Should be\n"+'\033[0m'+task7_str1+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			...
			gets = telnet_client2.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server output)"+gets)
			assert (gets == self.str3), '\033[91m'+"task7-1 Failed. Should be\n"+'\033[0m'+self.str3+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'	
			print("\33[3m"+"(Client1 side input) " +"\33[0m"+"3 2 1")
			telnet_client2.write(b"3 2 1\n")
			gets = telnet_client2.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server output)"+gets)
			assert (gets == task7_str2), '\033[91m'+"task7-1 Failed. Should be\n"+'\033[0m'+task7_str2+'\033[91m' +'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
		except AssertionError as e:
			#raise
			print(e)
		else:
			print("\33[32m"+"===task7-1 succeed!==="+"\33[0m")
		finally:
			write_proc.kill()
			write_proc.wait()	
			read_proc.kill()
			read_proc.wait()				
	def task8(self):
		task8_str1 = "Your preference order is AZ > BNT > Moderna.\n"
		task8_str2 = "Preference order for 902001 modified successed, new preference order is Moderna > BNT > AZ.\n"
		task8_str3 = "Your preference order is Moderna > BNT > AZ.\n"
		task8_str4 = "Preference order for 902001 modified successed, new preference order is AZ > BNT > Moderna.\n"
		read_proc = self.start_server("./read_server", self.read_port)
		write_proc = self.start_server("./write_server", self.write_port)
		try:
			telnet_client = TelnetConnection("localhost", self.write_port)
			telnet_client = telnet_client.telnet()
			telnet_client2 = TelnetConnection("localhost", self.read_port)
			telnet_client2 = telnet_client2.telnet()
		except Exception as e:
			print(e)
		try:
			gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server output)"+gets)
			assert (gets == self.str1), '\033[91m'+"task8-1 Failed. Should be\n"+'\033[0m'+self.str1+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			
			print("\33[3m"+"(Client side input) " +"\33[0m"+"902001")
			telnet_client.write(b"902001\n")
			gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server output)"+gets)
			assert (gets == task8_str1), '\033[91m'+"task8-1 Failed. Should be\n"+'\033[0m'+task8_str1+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			
			gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server output)"+gets)
			assert (gets == self.str3), '\033[91m'+"task8-1 Failed. Should be\n"+'\033[0m'+self.str3+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			
			print("\33[3m"+"(Client side input) " +"\33[0m"+"3 2 1")
			telnet_client.write(b"3 2 1\n")
			gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server output)"+gets)
			assert (gets == task8_str2), '\033[91m'+"task8-1 Failed. Should be\n"+'\033[0m'+task8_str2+'\033[91m' +'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			...
			gets = telnet_client2.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server2 output)"+gets)
			assert (gets == task8_str3), '\033[91m'+"task8-1 Failed. Should be\n"+'\033[0m'+task8_str3+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			...
			print("\33[3m"+"(Client side input) " +"\33[0m"+"1 2 3")
			telnet_client.write(b"1 2 3\n")
			gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server output)"+gets)
			assert (gets == task8_str4), '\033[91m'+"task8-1 Failed. Should be\n"+'\033[0m'+task8_str4+'\033[91m' +'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			...
			print("\33[32m"+"===task8-1 succeed!==="+"\33[0m")
		finally:
			write_proc.kill()
			write_proc.wait()	
			read_proc.kill()
	...
	def task9(self):
		...
		task9_str1 = "Your preference order is AZ > BNT > Moderna.\n"
		task9_str2 = "Preference order for 902001 modified successed, new preference order is Moderna > BNT > AZ.\n"

		read_proc = self.start_server("./read_server", self.read_port)
		read_proc2 = self.start_server("./read_server", self.read_port2)
		...
		try:
			gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server output)"+gets)
			assert (gets == self.str1), '\033[91m'+"task9-1 Failed. Should be\n"+'\033[0m'+self.str1+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			...
			gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
			print("\33[3m"+"(Server output)"+gets)
			assert (gets == task9_str1), '\033[91m'+"task9-1 Failed. Should be\n"+'\033[0m'+task9_str1+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			...
			print("\33[32m"+"===task9-1 succeed!==="+"\33[0m")
		finally:
			read_proc.kill()
			read_proc.wait()	
			read_proc2.kill()
...
def main():
	...
	try:
		...
		try:
			...
			cmd = subprocess.Popen(["cp", "registerRecord_copy", "registerRecord"])
		try:
			print('\33[1m'+"\n=== TASK7-1: A write server handles requests correctly. ==="+"\33[0m")
			scorer.task7()
		except Exception as e:
			raise
		finally:
			cmd = subprocess.Popen(["cp", "registerRecord_copy", "registerRecord"])
		try:
			print('\33[1m'+"\n=== TASK8-1: A write server handles requests correctly. ==="+"\33[0m")
			scorer.task8()
		except Exception as e:
			raise
		finally:
			cmd = subprocess.Popen(["cp", "registerRecord_copy", "registerRecord"])
		try:
			print('\33[1m'+"\n=== TASK9-1: A write server handles requests correctly. ==="+"\33[0m")
			scorer.task9()
		except Exception as e:
			raise	
	finally:
